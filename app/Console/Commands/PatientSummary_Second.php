...
class PatientSummary_Second extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'PatientSummary_Second:cron';
        ...
        public function handle()
    {
        ...
        $ScheduleData=\DB::table('patient_summary_report')->select('ps_report_id')->where(['is_active' => 1,'frequency' => 1,'schedular' => 2])
        ...
        if(empty($ScheduleData[0])){
            ...
            if(!empty($Patient_arr)){
                DB::table('patient_summary_report')->where('ps_report_id', $res[0]->ps_report_id)->update(['status' => 1,'schedular' => 2,'response' => "Started"]);
