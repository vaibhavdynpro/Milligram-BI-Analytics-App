	...
	<section class="content">
		<div class="container-fluid">
			<form role="form" method="post" action="{{ route('updatePhm',[$id]) }}">
				...
				<div class="row">
				<div class="col-4">
					<b>Report Name: </b><input id="name" type="text" name="name" value="{{$phmData->name}}" required placeholder="Enter Report Name" />
				</div>
				<div class="col-4">
			    <b>Client Name: </b>
			    <select  name="client_id" id="client_id" style="width: 40%; height: 100%;" data-placeholder="Select a client" required >
			        ...
			        <option value="{{$folder['id']}}" @php if($phmData->client_id == $folder['id']) echo 'selected'; else echo 'style="display:none"' @endphp > {{$folder['name']}}</option>
			        @endforeach						  
			    ...
			    <div class="col-4">
						<b>Report type: </b>
						<select  name="report_type" id="report_type" style="width: 40%; height: 100%;" data-placeholder="Select a client" required >
							<option value="" >Select Type</option>
							@foreach($reporttypes as $types)
							@if($phmData->report_type == $types->report_type_id)
							<option value="{{$types->report_type_id}}" selected>{{$types->report_type}}</option>
							@else
          ...
          @endphp
				...
				@php
				$sect_counter = 1;
				$sect_total_count = count($phmSectionDataArr);
				@endphp
        @foreach($phmSectionDataArr as $phmSection)
							
				<div class="card card-widget card-outline card-primary collapsed-card" id="section_box_{{$phmSection['section']->section_no}}" >
					<div class="card-header">
						<div class="user-block">
							<b>Section {{$phmSection['section']->section_no}}: </b><input id="section_heading_{{$phmSection['section']->section_no}}_1" size="70" value="{{$phmSection['section']->section_title}}" type="text" name="section_heading_{{$phmSection['section']->section_no}}_1" placeholder="Enter value" />
						</div>
						<!-- /.user-block -->
						<div class="card-tools">
						@if ($sect_total_count != $sect_counter)
							<a type="button" class="btn btn-tool" href="../../phm/addSectionBelow/{{$phmData->id}}/{{$phmSection['section']->section_no}}" data-toggle="tooltip" data-action="remove-old-section" title="Add New Section Below">
							Add Below</a>
						@else
						...
						$sect_counter++;
						@endphp
							<button type="button" class="btn btn-tool" data-card-widget="collapse" data-toggle="tooltip" title="Expand/Minimise" ><i class="fas fa-plus"></i>
							</button>
					...
					<div class="card-body">
						...
						@php
						$sub_sect_counter = 1;
						$sub_sect_total_count = count($phmSection['subSection']);
						@endphp
			@foreach($phmSection['subSection'] as $phmSubSection)
							...
							<div class="card card-widget card-outline card-warning collapsed-card" id="sub_section_box_{{$phmSection['section']->section_no}}_{{$phmSubSection->sub_section_no}}">
								<div class="card-header">
									...
									<div class="card-tools">
									@if ($sub_sect_total_count != $sub_sect_counter)
									<a type="button" class="btn btn-tool" href="../../phm/addSubSectionBelow/{{$phmData->id}}/{{$phmSubSection->section_id}}/{{$phmSubSection->sub_section_no}}" data-toggle="tooltip" data-action="remove-old-section" data-id="{{$phmSection['section']->section_no}}" title="Add New Section Below">Add Below</a>
									@else	
								...
								<div class="card-body">
									...
									<div class="file-upload-content" id="file_upload_content_{{$phmSection['section']->section_no}}_{{$phmSubSection->sub_section_no}}">
										<img class="file-upload-image" id="file_upload_image_{{$phmSection['section']->section_no}}_{{$phmSubSection->sub_section_no}}" data-id="{{$phmSection['section']->section_no}}_{{$phmSubSection->sub_section_no}}" src="{{$phmSubSection->look_img_url}}" alt="your image" />
        ...
        @endforeach
				...
				<a href="/reports" onclick="return confirm('You may have unsaved changes,Please save and close!!')" class="btn btn-default">Close </a>
			</form>
			<!-- Modal -->
	...
	<div class="modal fade" id="modal-danger">
        <div class="modal-dialog">
          <div class="modal-content bg-danger">
            ...
            <div class="modal-body">
              <p>You may lose the unsaved changes,Please click cancel and save the changes before proceding.</p>
            </div>
            <div class="modal-footer justify-content-between">
              <button type="button" id="cancel" class="btn btn-outline-light" data-dismiss="modal">Cancel</button>
              <button type="button" id="ok" data-dismiss="modal" class="btn btn-outline-light">Continue</button>
            </div>
          </div>
          <!-- /.modal-content -->
        </div>
...
<script type="text/javascript">
   ...
   locale: {
     ...
     //  CKEDITOR.replace( 'messageArea',
         ...
         //   toolbar : 'simple'
         //   })config.removeButtons = 'Underline,JustifyCenter';  //removePlugins
		 //CKEDITOR.config.removePlugins = 'forms';
		 //CKEDITOR.config.removeButtons = 'Styles,About,Image,Flash,HorizontalRule,Smiley,PageBreak,Iframe,Anchor,Save,NewPage,ExportPdf,Print,Templates,Cut,Copy,Paste,PasteText,PasteFromWord,Scayt,Form,Checkbox,Radio,TextField,Textarea,Select,Button,ImageButton,HiddenField,Strike,Blockquote,CreateDiv,Language';
   ...
   $( document ).ready(function() {
		  // alert(localStorage.getItem("sect_id"));
		  var sect_no = localStorage.getItem("sect_id");
		  $("#section_box_"+sect_no).removeClass("collapsed-card");
		  localStorage.removeItem("sect_id");
	});
</script> 
@endsection
